...
import { Users, Plus, Edit, Trash2, Shield, Check, X, AlertCircle } from 'lucide-react';
...
const ClientsPage: React.FC = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      const possibleRoutes = ['/clients', '/api/clients', '/api/v1/clients'];
      let response;
      ...
      for (const route of possibleRoutes) {
        try {
          ...
          // Continue avec la route suivante si 404
        }
      }
      ...
      if (response) {
        ...
        throw new Error('Aucune route API valide trouvée');
      }
    } catch (error: any) {
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Essayer différentes routes pour l'ajout
      const possibleRoutes = ['/clients', '/api/clients', '/api/v1/clients'];
      let response;
      
      for (const route of possibleRoutes) {
        try {
          response = await api.post(route, newClient);
          break;
        } catch (err: any) {
          if (err.response?.status !== 404) {
            throw err;
  ...
  const handleStatusChange = async (clientId: string, newStatus: 'active' | 'inactive') => {
    try {
      ...
      for (const route of possibleRoutes) {
        try {
          if (route.includes('/status')) {
            await api.patch(route, { status: newStatus });
          } else {
            await api.put(route, { status: newStatus });
          }
          success = true;
          break;
        } catch (err: any) {
      ...
      setError(error.response?.data?.message || 'Erreur lors de la modification du statut');
    }
  };

  // Fonction pour fermer la modal et réinitialiser
